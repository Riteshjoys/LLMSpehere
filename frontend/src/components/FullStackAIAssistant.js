...
const FullStackAIAssistant = () => {
  ...
  const fetchProjects = async () => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch('/api/fullstack-ai/projects', {
        headers: { 'Authorization': `Bearer ${token}` }
      });
  ...
  const createProject = async (e) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch('/api/fullstack-ai/project/initialize', {
        method: 'POST',
        headers: {
  ...
  const resumeProject = async (projectId) => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`/api/fullstack-ai/project/${projectId}/resume`, {
        method: 'POST',
        headers: { 'Authorization': `Bearer ${token}` }
      });
